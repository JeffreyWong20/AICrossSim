...
from submodules.mase.src.chop.nn.snn.modules.sta import SpikeAttention, SpikeLN, SpikeLinear_ReLU, TransformRelu, StraightThrough
...
def reset_model(model):
    ...
    for name, child in children:
        is_need = False
        if (
            isinstance(child, SpikeAttention)
            or isinstance(child, SpikeLN)
            or isinstance(child, SpikeLinear_ReLU)
            or isinstance(child, TransformRelu)
        ):
